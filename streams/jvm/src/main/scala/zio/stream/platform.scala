...
trait ZStreamPlatformSpecificConstructors {
  ...
  // ReadMode = Complete | Range(Long, Long) | Tail(startAt: Option[Long], pollDuration: Duration)

  trait ReadMode
  object Complete extends ReadMode
  case class Range(start:Long, end:Long) extends ReadMode
  case class Tail(startAt: Option[Long], pollDuration: Duration)
